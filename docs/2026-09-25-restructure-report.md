...
| D8 | **演示内容移出框架** | demo11 纵切面、Demo011Scheduler、`AuthPermissionDemoSeed011` 全部归 reference-app；框架制品不再含演示账号播种 |
...
| Starter | 必选性 | 缺席时 |
|---|---|---|
| security-starter | 必选（熔断） | web 应用拒绝启动 |
| data-mybatis-starter | 必选 | 数据访问 kernel 缺失 |
| scheduler-quartz-starter | 可选 | 调度端点整体 404（路由级），system011 其余照常 |
| ai-starter | 可选 | 推理/TTS/ASR/图片端点 404；提示词管理（julyAiDomain）保留 |
| storage-starter011 | 可选（用存储时必引） | 正常启动，使用时才报解析失败 |
| message-starter | 可选 | 正常启动，发送时才报无渠道 |
| observability-starter | 可选 | 无 actuator/指标/traceId MDC |
...
README（架构总览/模块表/装配说明）· 015.project-info（Maven 骨架/运行命令）· 020 quickstart（依赖模型/main 模板/必选可选表/pitfalls）· infrastructure011/013（结构全篇）· 015.config（krt 键拆分）· 020.deploy（产物/日志）· 017 红线（卫兵实现类）· 033（密钥位表述）· storage/message 用法篇（BOM 片段）· 017.datasource-center（模块归属图）· requirement 两源 13 篇（映射 + 注记）· script011.sh / deploy.sh / doc-checker / logback / spring.application.name（5 处硬编码）。

## 八、遗留（P0，未夹带处理）

1. **演示播种**：每次启动重置全部演示账号密码（含超管）为公开口令并写日志——加环境门槛、去重置、去日志。
2. **功能级鉴权仍是 opt-in**：未挂 `assertHas` 的接口不做权限校验（用户/角色/密码重置/数据源/调度等）——改默认拒绝 + 显式声明，见审计报告与 018 IAM 文档。
3. **同步链路可写主库 `july_user`**（SyncSink 无目标表白名单）；`SqlGuard011` 缺 `INTO OUTFILE`/`LOAD_FILE` 禁词。
4. **凭据治理**：入库开发配置含明文凭据与可预测 JWT secret（需轮换 + 历史清理 + Gitleaks 进 CI）；production 模板改占位符。
5. 工程债：gate 不跑测试（加 `mvn test` 一行）；Druid 探测池泄漏；消息中心克隆家族；starter 条件装配进一步完整化（`@ConditionalOnMissingBean` 支持消费方覆写）。
