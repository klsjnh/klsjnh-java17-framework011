...
--       july_user.sql / july_perm.sql（2026-09-24，按中心归并；文件名 july_center_iam）
-- 列顺序规范：id → 业务字段 → sort_order（有排序需求时）→ status → 审计四列 → dr
-- 设计：docs/infrastructure011/018.iam-center/（011 设计 / 013 架构 / 015 用法 / 016 动态权限）
--       docs/requirement011/011 菜单 · 013 组织 · 015 用户 · 016 角色
-- 组成：组织树 · 菜单树 · 角色(+权限) · 用户(+角色关联/审计) · 权限目录(对象/动作)
-- 种子：目录与演示账号由 Java Seed Runner 幂等写入（JulyPermCatalogSeed011 /
--       AuthPermissionDemoSeed011），不在本文件 INSERT。
-- 表顺序：无库级 FK；按领域依赖排列（组织/菜单/角色 → 用户及关联 → 权限目录）。
-- ============================================================

-- ------------------------------------------------------------
-- 组织机构（树；负责人挂 july_user，可空）
-- ------------------------------------------------------------
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='IAM 中心 - 权限对象目录';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='IAM 中心 - 权限动作目录';
