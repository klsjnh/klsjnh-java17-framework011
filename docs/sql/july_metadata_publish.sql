...
ALTER TABLE july_metadata
    ...
    ADD COLUMN physical_table    VARCHAR(60)  NULL                    COMMENT '物理表名（lc_ 前缀）',
    ADD COLUMN backup_table      VARCHAR(60)  NULL                    COMMENT '最近一次发布前的备份表名',
...
CREATE TABLE IF NOT EXISTS july_metadata_version (
    ...
    physical_table  VARCHAR(60)  NOT NULL                COMMENT '物理表名（lc_ 前缀）',
    backup_table    VARCHAR(60)  NULL                    COMMENT '备份表名（迁移前 rename）',
