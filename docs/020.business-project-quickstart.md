...
| `*-infrastructure011` | PO / Mapper / adapters | domain + `java17-core011` + `java17-data-mybatis-starter011` |
| `*-web011` | Controllers / VOs | application + `java17-core011` |
| `*-app011` | **Only** `main` + `application*.yml` | framework starters (`java17-security-starter011`, `java17-data-mybatis-starter011`, …) |

Framework modules are consumed as **BOM + starters**: import `java17-bom011`, then add the starters you need (`java17-core011` is pulled in transitively). See 015.project-info for the full 14-module map.
...
| `java17-data-mybatis-starter011` | **required** (dynamic datasource kernel + framework mapper scan) | — |
| `java17-scheduler-quartz-starter011` | optional | scheduler endpoints disappear entirely (404), the rest of system011 keeps working |
| `center-ai011-starter` | optional | inference / tts / asr / image endpoints disappear; prompt management (`julyAiDomain`) stays |
| `center-storage011-starter` | optional (required when storage is used) | app boots; resolving a storage instance fails only when actually used |
| `center-message011-starter` | optional | app boots; sending fails only when actually used |
| `java17-observability-starter011` | optional | no actuator / metrics / traceId MDC |
