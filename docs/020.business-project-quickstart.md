...
框架仓库外的兄弟目录 **`java17-demo011-app011`** 是一个已验证可跑的消费方演示工程：BOM + `java17-security-starter011` / `java17-data-mybatis-starter011` / `java17-scheduler-quartz-starter011`，业务纵切面 `com.klsjnh.demo011`（五层照抄本手册）+ 消费方定时任务（`Demo011Job` 实现 `JobHandler` SPI）。构建前提：框架仓库先 `mvn -o clean install`；业务表 `sql/july_demo011.sql`；端口 11161。E2E 已验证（登录 / CRUD 五步 / 调度 executeTimes 0→2，2026-09-25）。
