...
/**
 * Read-purpose action names: these are safe + idempotent, so the handler MUST
 * be mapped with @GetMapping (RFC 9110 semantic contract).
 */
const READ_ACTIONS = new Set(['getById', 'selectListByPage', 'selectTree', 'selectUserMenuTree', 'selectList']);
...
export function testApiUrlStandards(path, content, violations) {
  ...
  // Rule B (new): HTTP method must match the action semantics (RFC 9110).
  // Read actions (safe + idempotent) → @GetMapping; write actions → @PostMapping.
  ...
  while ((match = mappingPattern.exec(content)) !== null) {
    ...
    if (isRead && httpMethod !== 'GET') {
      violations.push({
        file: path,
        line: lineOf(content, match.index),
        rule: 'api-method',
        detail: `read action "${action}" must be @GetMapping, found @${httpMethod}Mapping`,
        fix: 'read actions are safe + idempotent — map them with @GetMapping and pass parameters via the query string',
      });
