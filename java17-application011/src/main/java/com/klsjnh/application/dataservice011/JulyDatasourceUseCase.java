package com.klsjnh.application.dataservice011;
...
public class JulyDatasourceUseCase {
    ...
    /**
     * Batch logic delete with a per-id success/failure summary, then one
     * registry refresh for the whole batch.
     *
    ...
    private void requireDbType(String dbType) {
        if (DatabaseType011.fromString(dbType) == null) {
            throw BusinessException.badRequest("unknown database type: " + dbType);
        }
    }
