...
/*                JulyInboundTemplateRepositoryImpl class
 *
...
import com.klsjnh.infrastructure.messagecenter.inbound.mapper.JulyInboundTemplateMapper;
...
/**
 * Repository implementation for the JulyInboundTemplate aggregate on the base
 * repository (july_message_inbound_template, business unique column template_code).
 */

@Repository
public class JulyInboundTemplateRepositoryImpl
        extends BaseRepository<JulyInboundTemplatePo, JulyInboundTemplateMapper>
        implements JulyInboundTemplateRepository {
     ...
     */
    public JulyInboundTemplateRepositoryImpl(JulyInboundTemplateMapper mapper, CommonMapper commonMapper) {
    ...
    protected String getTableName() {
        return "july_message_inbound_template";
    }
     ...
     */
    @Override
    protected Object getBusinessValue(JulyInboundTemplatePo entity) {
     ...
     */
    @Override
    public void insert(JulyInboundTemplate template) {
     ...
     */
    @Override
    public void update(JulyInboundTemplate template) {
     ...
     */
    @Override
    public JulyInboundTemplate findById(String id) {
        JulyInboundTemplatePo po = getById(id);
     ...
     */
    @Override
    public JulyInboundTemplate findByCode(String templateCode) {
        JulyInboundTemplatePo po = getByBusinessValue(templateCode);
     ...
     */
    @Override
    public JulyInboundTemplate findEnabledByCode(String templateCode) {
        QueryWrapper<JulyInboundTemplatePo> wrapper = new QueryWrapper<>();
        wrapper.eq("template_code", templateCode)
                .eq("status", Status011.ENABLED.getCode())
                .last("LIMIT 1");

        JulyInboundTemplatePo po = mapper.selectOne(wrapper);
     ...
     */
    @Override
    public List<JulyInboundTemplate> findPage(int offset, int pageSize, JulyInboundTemplateQuerySpec spec) {
        int current = offset / pageSize + 1;
        Page<JulyInboundTemplatePo> page = Page.of(current, pageSize);
     ...
     */
    @Override
    public long count(JulyInboundTemplateQuerySpec spec) {
     ...
     */
    private QueryWrapper<JulyInboundTemplatePo> specWrapper(JulyInboundTemplateQuerySpec spec) {
        QueryWrapper<JulyInboundTemplatePo> wrapper = new QueryWrapper<>();
        JulyInboundTemplateQuerySpec query = spec == null ? new JulyInboundTemplateQuerySpec(null, null, null) : spec;
     ...
     * @return PO
     */
    private JulyInboundTemplatePo toPo(JulyInboundTemplate template) {
        JulyInboundTemplatePo po = new JulyInboundTemplatePo();
     ...
     * @return aggregate
     */
    private JulyInboundTemplate toAggregate(JulyInboundTemplatePo po) {
        AuditInfo audit = new AuditInfo(po.getCreateBy(), po.getUpdateBy(), po.getCreateTime(), po.getUpdateTime());

        return new JulyInboundTemplate(EntityId.of(po.getId()), po.getTemplateCode(), po.getSortOrder(),
