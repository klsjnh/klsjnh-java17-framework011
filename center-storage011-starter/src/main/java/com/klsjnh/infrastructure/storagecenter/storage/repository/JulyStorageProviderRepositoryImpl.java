...
public class JulyStorageProviderRepositoryImpl
    ...
    /**
     * Reversible cipher for access/secret keys.
     */
    ...
    public JulyStorageProviderRepositoryImpl(JulyStorageProviderMapper mapper, CommonMapper commonMapper,
            JulyStorageProviderBucketRepositoryImpl bucketRepository, SecretCipherPort secretCipher) {
...
/**
    ...
    private JulyStorageProviderPo toPo(JulyStorageProvider storage) {
        ...
        po.setAccessKey(secretCipher.encrypt(storage.accessKey()));
        po.setSecretKey(secretCipher.encrypt(storage.secretKey()));
    ...
    private JulyStorageProvider toAggregate(JulyStorageProviderPo po) {
        ...
        return new JulyStorageProvider(EntityId.of(po.getId()), po.getStorageCode(), po.getSortOrder(),
                po.getStorageName(), po.getProvider(), po.getBasePath(), po.getEndpoint(),
                secretCipher.decrypt(po.getAccessKey()), secretCipher.decrypt(po.getSecretKey()),
                "1".equals(po.getSecure()),
                presign, po.getRemark(), po.getStatus(), audit);
    }
}
